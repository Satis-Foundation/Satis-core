...
/**
 * This contract is a simple ETH pool for deposit.
 * It supports transfer and withdrawal of assets.
 */


contract EthPoolRaw {

    mapping (string => uint256) public instantWithdrawEthReserve;
    mapping (address => uint256) public clientEthNonce;
    ...
    event WorkerTakeLockedEth(address workerAddress, uint256 takeValue);
    event WorkerDumpBridgedEth(address workerAddress, address[] clientAddressList, uint256[] dumpValueList);
    event WorkerDumpInstantWithdrawEth(address workerAddress, address[] _clientAddressList, uint256[] _instantWithdrawValueList);
    event OwnerTakeProfit(uint256 takeProfitValue);
     ...
     */
    function getInstantWithdrawEthReserve(string[] memory _ticketIdList) public view returns(uint256[] memory) {
        uint256[] memory reserveValueList = new uint256[](_ticketIdList.length);
        for (uint i = 0; i < _ticketIdList.length; i++) {
            reserveValueList[i] = (instantWithdrawEthReserve[_ticketIdList[i]]);
        }
     ...
     */
    function addFundWithAction(address _clientAddress, uint256 _addValue) external payable isProxy returns(bool _isDone) {
        _isDone = true;
    }

...
const zkc1Address = "0x98035Cd0e77A5b782647aa228cEe32cc40A29DF6";
const timAddress = "0xBc122F3f0b447AD5310b3306C3562843A0e78F8e";
//const timAddress = "0xc4adcF8814a1da13522716A23331Ce4d48A1414d";
...
//   console.log();
  ...
  // Check contract free balance
  const liquidityAmountOld = await rawPool.getLiquidityAmountInPool(zkc1Address);
  console.log(`Current free assets in raw pool: ${liquidityAmountOld}`);

  // Record Tim's balance
  const timBalanceOldZKC1 = await zkc1.balanceOf(timAddress);
  console.log(`Tim has ${timBalanceOldZKC1} ZKC1 before`);

//   // Tim queue withdrawal
//   var withdrawNonce = await rawPool.getClientNonce(timAddress);
//   console.log(`Tim's current nonce: ${withdrawNonce}`);
//   const withdrawHash = "0xaaf2c0c04bb54747db784a8421aa83061507651d8a5cafdc5a540786f6b059521aa1dcc5be1b4d7ac2c3cd4c4edcc086a7cdfa506fa55546f7c54de8b56a9fad1b";
//   const withdrawTx = await proxyContract.verifyAndWithdrawFund(withdrawHash, zkc1Address, 1030, 1, 280, rawPoolAddress, withdrawNonce, "test");
//   await withdrawTx.wait();
//   console.log(`Withdraw hash: ${withdrawTx.hash}`);

  // Worker approves
  const approveTx = await zkc1.approve(rawPoolAddress, 100);
  await approveTx.wait();
  console.log(`Approve hash: ${approveTx.hash}`);

  // Release funds
  const dumpTx = await rawPool.workerDumpInstantWithdrawalFund([timAddress], zkc1Address, [600], 100);
  await dumpTx.wait();
  console.log(`Dump TX hash: ${dumpTx.hash}`);

  // Check Tim's balance again
  const timBalanceNewZKC1 = await zkc1.balanceOf(timAddress);
  console.log(`Tim has ${timBalanceNewZKC1} ZKC1 now`);
  console.log(`He gains ${timBalanceNewZKC1 - timBalanceOldZKC1}`);

  const liquidityAmountNew = await rawPool.getLiquidityAmountInPool(zkc1Address);
  console.log(`Current free assets in raw pool: ${liquidityAmountNew}`);
  console.log(`Difference: ${liquidityAmountNew - liquidityAmountOld}`);
}
...
async function main() {
    ...
    const tier1WithdrawTx = await proxy.connect(worker2).verifyAndWithdrawFund(signature, token_address, withdraw_final, tier, nonce, "satis-v2");
    ...
    tier = 3;
    signature = await withdrawSignature(nonce, client_address, token_address, withdraw_final, tier, chain_id, pool_address);
    const tier3WithdrawTx = await proxy.connect(worker2).verifyAndPartialWithdrawFund(signature, token_address, withdraw_final, tier, nonce, "satis-v2");
    ...
    var workerUnlockTx = await rawPool.connect(worker1).workerUnlockFund([worker2.address], token.address, [400], 2);

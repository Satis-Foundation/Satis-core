...
async function main() {
    ...
    var tier1WithdrawTx = await proxy.connect(worker1).verifyAndWithdrawFund(signature, token_address, withdraw_final, tier, nonce, "satis-v2");
    ...
    console.log(`Signing withdraw signature:`)
    var signature = await withdrawSignature(nonce, client_address, token_address, withdraw_final, tier, chain_id, pool_address);
    var tier1WithdrawTx = await proxy.connect(worker2).verifyAndWithdrawFund(signature, token_address, withdraw_final, tier, nonce, "satis-v2");
    await tier1WithdrawTx.wait();

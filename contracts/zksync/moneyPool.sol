...
contract moneyPool {
    ...
    modifier enoughMobileBalance(address _tokenAddress, uint256 _tokenValue) {
        require (
            _tokenValue <= clientBalance[msg.sender][_tokenAddress] - clientLockBalance[msg.sender][_tokenAddress],
     ...
     */
    function addFund(address _tokenAddress, uint256 _tokenValue) public {
        IERC20 depositToken = IERC20(_tokenAddress);
        depositToken.safeTransferFrom(msg.sender, address(this), _tokenValue);
        if (clientBalance[msg.sender][_tokenAddress] > 0) {
            clientBalance[msg.sender][_tokenAddress] += _tokenValue;
        } else {
            clientBalance[msg.sender][_tokenAddress] = _tokenValue;
        }
    ...
    function lockFundWithAction(address _tokenAddress, uint256 _tokenValue, string memory _data) public enoughMobileBalance(_tokenAddress, _tokenValue) {
        if (clientLockBalance[msg.sender][_tokenAddress] > 0) {
            // clientLockBalance[msg.sender][_tokenAddress] = clientLockBalance[msg.sender][_tokenAddress].add(_tokenValue);
            clientLockBalance[msg.sender][_tokenAddress] += _tokenValue;
            // clientLockBalance[msg.sender][_tokenAddress] = 1;
        } else {
            clientLockBalance[msg.sender][_tokenAddress] = _tokenValue;
        }
     ...
     */
    function addFundWithAction(address _tokenAddress, uint256 _lockValue, uint256 _addValue, string memory _data) external {
    ...
    function unlockFund(address _clientAddress, address _tokenAddress, uint256 _tokenValue) public isOwner {
        clientLockBalance[_clientAddress][_tokenAddress] -= _tokenValue;
    ...
    function verifyAndUnlockFund(bytes memory _targetSignature, address _tokenAddress, uint256 _unlockValue, uint256 _nonce, uint256 _newLockValue) public {
        ...
        clientNonce[msg.sender] = _nonce + 1;

        // unlockFund
        uint256 oldLock = clientLockBalance[msg.sender][_tokenAddress];
        uint256 _diff;
        if (_newLockValue >= oldLock) {
          _diff = _newLockValue - oldLock;
          if (clientBalance[msg.sender][_tokenAddress] > 0) {
            clientBalance[msg.sender][_tokenAddress] += _diff;
          } else {
            clientBalance[msg.sender][_tokenAddress] = _diff;
          }
        } else {
          _diff = oldLock - _newLockValue;
          clientBalance[msg.sender][_tokenAddress] -= _diff;
        }
        ...
        clientLockBalance[msg.sender][_tokenAddress] -= _unlockValue;
        emit Unlock(msg.sender, _tokenAddress, _unlockValue);
    }

    // function verify(bytes memory _targetSignature, address _tokenAddress, uint256 _unlockValue, uint256 _nonce, uint256 _newLockValue) public returns(uint256 oldLock, uint256 _diff) {
    //     // require(clientNonce[msg.sender] == _nonce, "Invalid nonce");
    //     bytes32 _matchHash;
    //     bytes32 _hashForRecover;
    //     address _recoveredAddress;
    //     Str memory str;
    //     str.sender = "0x29c624f78a8562bd21759cb8cac4f043f09c143e";
    //     str.token = address2str(_tokenAddress);
    //     str.unlock = uint2str(_unlockValue);
    //     str.nonce = uint2str(_nonce);
    //     str.newLock = uint2str(_newLockValue);
    //     // _raw = string(abi.encodePacked("newlock:", str.newLock, ";", str.nonce, str.sender, str.token, str.unlock));
    //     _matchHash = keccak256(abi.encodePacked("newlock:", str.newLock, ";", str.nonce, str.sender, str.token, str.unlock));
    //     // _mid = _matchHash;
    //     // require (_targetHash == _matchHash, "Incorrect hash");
    //     _hashForRecover = hashingMessage(_matchHash);
    ...
    //     _recoveredAddress = recoverSignature(_hashForRecover, _targetSignature);
    //     // _owner = _recoveredAddress;
    //     // require (_recoveredAddress == owner, "Incorrect signature");
    //     // clientNonce[msg.sender] = _nonce.add(1);

    //     // unlockFund
    //     clientBalance[msg.sender][_tokenAddress] = 500e6;
    //     clientLockBalance[msg.sender][_tokenAddress] = 500e6;
    //     oldLock = clientLockBalance[msg.sender][_tokenAddress];
    //     // uint256 _diff;
    //     if (_newLockValue >= oldLock) {
    ...
    function ownerTakeLockedFund(address _clientAddress, address _tokenAddress, uint256 _tokenValue) public isOwner {
        ...
        clientLockBalance[_clientAddress][_tokenAddress] -= _tokenValue;
        clientBalance[_clientAddress][_tokenAddress] -= _tokenValue;

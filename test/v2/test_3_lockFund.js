...
describe ("Check Add then Lock", function() {
    it ("Check seperate add and lock", async function() {
        ...
        await token.connect(user2).approve(pool1Addr,5000000);
        ...
        await mainPool.connect(user2).lockFundWithAction(tokenAddress,2500000,'lockTest','1pool');
        balanceValue = await mainPool.connect(user2).getClientBalance(user2['address'],tokenAddress,"1pool");
        var lockedValue = await mainPool.connect(user2).getClientLockBalance(user2['address'],tokenAddress,"1pool");
